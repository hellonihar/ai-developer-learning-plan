...
def get_default_params(algorithm_name):
    params = {
        "KMeans (sklearn)": {"n_clusters": 3},
        "KMeans (scratch)": {"n_clusters": 3},
        "DBSCAN": {"eps": 0.5, "min_samples": 5},
        "Hierarchical (Agglomerative)": {"n_clusters": 3, "linkage": "ward"},
    }
    return params.get(algorithm_name, {})


def run_clustering(algorithm_name, df, feature_cols, **kwargs):
    X = df[feature_cols].values
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X)

    func = ALGORITHMS[algorithm_name]
    labels, model = func(X_scaled, **kwargs)

    return labels, model, X_scaled, scaler
